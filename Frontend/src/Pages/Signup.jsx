...
import { postsignupUser } from "../Redux/AuthReducer/action";
...
function Signup() {
  ...
  const handleSubmit = (event) => {
    ...
    if (password.trim() === "") {
      ...
      dispatch(postsignupUser({ name, email, password }))
        .then((res) => {
          if (res.payload[0].message === "Signup Successfully") {
            toast({
  ...
  return (
    <Stack align={"center"} bgColor={"blackAlpha.50"} height={"99vh"}>
      ...
      >
        ...
        <InputGroup pb={isNameEmpty ? "0px" : 4} border={"0px solid red"}>
          <VStack alignItems="flex-start" width="100%">
            <FormLabel
              color={isNameEmpty ? "red" : "grey"}
              mb={"-7px"}
              fontSize={"16px"}
              fontWeight="thick"
            >
