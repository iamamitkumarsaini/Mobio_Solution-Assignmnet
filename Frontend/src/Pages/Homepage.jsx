...
function Homepage() {
  ...
  return (
    <Stack bg={"blackAlpha.100"}>
      <Navbar />
      <Stack width={"98%"} m={"auto"} mt={"100px"} bg={"blackAlpha.100"}>
        <Flex
          align={"center"}
        ...
        >
          ...
          <Box display={["block", "block", "none"]}>
            ...
            >
              <ModalOverlay />
              <ModalContent position="fixed" maxW={["90%", "60%"]} top={[16]}>
        ...
        >
          {products?.map((elem) => {
            const isAddToCart =
              ...
              getCartData?.some((ele) => ele.product_id === elem._id);

            return (
              <GridItem key={elem._id} gap={3} boxShadow={"md"} bg={"#fff"}>
                <Box p={4} height={"300px"}>
                  <Image
                    objectFit="contain"
                    width="100%"
                    height="100%"
